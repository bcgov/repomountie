...
export const issueCommentCreated = async (context: Context): Promise<void> => {
  ...
  if (isFromBot) {
    // Don't act crazy.
    logger.info(
      `Skipping issue ${context.payload.issue.id} because its from a bot`
    );
    return;
  }
...
export const repositoryScheduled = async (
  context: Context,
  scheduler: any
): Promise<void> => {
  ...
  if (!ACCESS_CONTROL.allowedInstallations.includes(owner)) {
    logger.info(
      `Skipping scheduled repository ${repo} because its not part of an allowed installation`
    );
    return;
  }
  ...
  try {
    ...
    await Promise.all([
      ...
      requestStatusBadgeIfRequired(context, owner, repo),
    ]);
  } catch (err) {
...
export const repositoryDeleted = async (
  context: Context,
  scheduler: any
): Promise<void> => {
  scheduler.stop(context.payload.repository);
